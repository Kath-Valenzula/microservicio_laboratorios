...
-- Drops seguros 
BEGIN EXECUTE IMMEDIATE 'DROP TABLE resultados_analisis CASCADE CONSTRAINTS'; EXCEPTION WHEN OTHERS THEN IF SQLCODE != -942 THEN RAISE; END IF; END;
/
...
CREATE TABLE resultados_analisis (
  ...
  tipo_examen    VARCHAR2(200) NOT NULL,
  fecha          DATE NOT NULL,
...
ALTER TABLE resultados_analisis ADD CONSTRAINT fk_resa_usu FOREIGN KEY (paciente_id) REFERENCES usuarios(id);
ALTER TABLE resultados_analisis ADD CONSTRAINT fk_resa_lab FOREIGN KEY (laboratorio_id) REFERENCES laboratorios(id_lab);

-- Índices sobre FKs
CREATE INDEX idx_res_usu ON reservas(id_usuario);
CREATE INDEX idx_res_lab ON reservas(id_lab);

CREATE INDEX idx_resa_paciente ON resultados_analisis(paciente_id);
CREATE INDEX idx_resa_lab ON resultados_analisis(laboratorio_id);
